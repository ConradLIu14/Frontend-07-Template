...
function layout(element){
    ...
    if(!style[crossSize]){// autoSizing
        ...
        for(var i=0;i<flexLine.length;i++){
            elementStyle[crossSize] = elementStyle[crossSize] + flexLine[i].corssSpace
        }
    }else{
    ...
    flexLine.forEach(function (irems){
        ...
        for(var i = 0;i<items.length;i++){
            ...
            if(align === 'stretch'){
                itemStyle[crossStart] = crossBase
                itemStyle[crossEnd] = crossBase + crossSign*((itemStyle[crossSize] !== null && itemStyle[crossSize]!==(void 0)))// ！！！ 这里可能没写对
